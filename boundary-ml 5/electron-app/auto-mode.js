class AutoMode {
  ...
  snapshot() { return {running:this.running, status:this.status, decision:this.decision, score:this.score??null,level:this.level??null,category:this.category??null,cycles:this.cycles||0,recognizedItems:this.recognizedItems??null,lastCheck:this.lastCheck??null,lastPublication:this.lastPublication??null,error:this.error??null}; }
  ...
  start() {
    ...
    this.running=true; this.lastText=null; this.alerted=false; this.decision=null;this.score=null;this.level=null;this.category=null;this.cycles=0;this.recognizedItems=null;this.lastCheck=null;this.lastPublication=null;this.error=null;
    this.pending=this.loop().finally(()=>{this.pending=null;});
  }
  stop(reason) { this.running=false;this.stopReason=reason||null; this.update(reason || (this.pending?'Stopping after the active stage…':'Stopped')); }
  async loop() {
    try {
      while(this.running) {
        ...
        if(!this.running) break;
        this.recognizedItems=messages.length;
        ...
        if(fingerprint!==this.lastText) {
          ...
          if(decision===true && !this.alerted) {
            ...
            this.alerted=true;
            this.lastPublication=new Date().toISOString();
        ...
        this.cycles++;
        this.ops.changed?.(this.snapshot());
        // Processing completion, not a fixed capture timer, starts the next cycle.
        await new Promise(resolve=>setImmediate(resolve));
      }
    } catch(error) { this.running=false;this.error=error.message; this.update(`Paused: ${error.message}`); }
    finally { if(!this.status.startsWith('Paused:')) this.update(this.stopReason||'Stopped'); }
  }
}
