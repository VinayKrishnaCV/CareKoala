...
public final class GuardianService extends Service {
 private volatile boolean running;private ExecutorService workers;
 ...
 private void status(String text){getSystemService(NotificationManager.class).notify(1,notice(text));}
 @Override public int onStartCommand(Intent intent,int flags,int id){
  ...
  try {
   ...
   workers.submit(()->listen(pair));
  }catch(Exception e){status("Pairing unavailable — open the app");stopSelf();}
  return START_NOT_STICKY;
 }
 private void listen(JSONObject pair){
  long retryMillis=1000;
  while(running){HttpURLConnection connection=null;
   try{
    ...
    try(Reader reader=new InputStreamReader(connection.getInputStream(),StandardCharsets.UTF_8)){
     ...
     while(running && (ch=reader.read())!=-1){
      if(ch=='\n'){accept(pair,line.toString());line.setLength(0);}
   ...
   catch(Exception error){if(running)status("Disconnected — retrying ntfy.sh connection");}
   finally{if(connection!=null){connections.remove(connection);connection.disconnect();}}
   if(running){try{Thread.sleep(retryMillis);}catch(InterruptedException e){Thread.currentThread().interrupt();break;}retryMillis=Math.min(30000,retryMillis*2);}
  }
 }
 private synchronized void accept(JSONObject pair,String record){
  try{
   if(!running || record.trim().isEmpty())return;
 ...
 @Override public void onDestroy(){running=false;for(HttpURLConnection c:connections)c.disconnect();if(workers!=null)workers.shutdownNow();stopForeground(STOP_FOREGROUND_REMOVE);super.onDestroy();}
}
